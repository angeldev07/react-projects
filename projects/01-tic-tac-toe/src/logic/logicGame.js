...
const markCell = ({index, turn, board, setBoard, setPlayer, player, winner, setWinner, score, setScore}) => {
...
const checkWinner = ({newBoard, setWinner, winner, score, setScore}) => {
  ...
  allWinners.forEach((winnerP) => {
    ...
    const [a, b, c] = winnerP;
    if (newBoard[a].turn && newBoard[a].turn === newBoard[b].turn && newBoard[a].turn === newBoard[c].turn) {
